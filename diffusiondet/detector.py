...
class DiffusionDet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.use_cbcr = cfg.INPUT.USE_CbCr_Proposals
        ...
        weight_step_dcit = cfg.MODEL.DiffusionDet.STEP_WEIGHT
        weight_dict = {"loss_ce": class_weight, "loss_bbox": l1_weight, "loss_giou": giou_weight}
        
        if self.deep_supervision:
            aux_weight_dict = {}
            for i in range(self.num_heads - 1):
                aux_weight_dict.update({k + f"_{i}": v * weight_step_dcit[i] for k, v in weight_dict.items()})
            weight_dict.update(aux_weight_dict)

        weight_dict.update({"loss_ce": 2 * weight_step_dcit[-1], "loss_bbox": 5 * weight_step_dcit[-1], "loss_giou": 3 * weight_step_dcit[-1]})
        losses = ["labels", "boxes"]
    ...
    def model_predictions(self, backbone_feats, images_whwh, x, t, x_self_cond=None, filenames=None, clip_x_start=False):
    ...
    def get_batch_CbCr_sample(self, vaild):
        CbCr_samples = list()
        for v in vaild:
            N, _ = v.shape
            ...
            if N >= self.num_proposals:
                sample = sample[:self.num_proposals]
                randwh = torch.randn((self.num_proposals, 2), device=self.device)
                CbCr_samples.append(torch.cat((v[sample].to(device=self.device), randwh), dim=1))
            else :
                randbox = torch.randn((self.num_proposals - N, 4), device=self.device)
                randwh = torch.randn((N, 2), device=self.device)
                cbcrs = torch.cat((v[sample].to(device=self.device), randwh), dim=1)
                CbCr_samples.append(torch.cat((cbcrs, randbox), dim=0))
            continue
        
        CbCr_samples = torch.stack(CbCr_samples)
        return CbCr_samples
    
    def get_CbCr_sample(self, vaild):
        N, _ = vaild.shape
        sample = torch.randperm(N)
        ...
        if N >= self.num_proposals:
            sample = sample[:self.num_proposals]
            randwh = torch.randn((self.num_proposals, 2), device=self.device)
            CbCr_samples = torch.cat((vaild[sample].to(device=self.device), randwh), dim=1)
        else :
            randbox = torch.randn((self.num_proposals - N, 4), device=self.device)
            randwh = torch.randn((N, 2), device=self.device)
            cbcrs = torch.cat((vaild[sample].to(device=self.device), randwh), dim=1)
            CbCr_samples = torch.cat((cbcrs, randbox), dim=0)
        
        return CbCr_samples

    @torch.no_grad()
    def ddim_sample(self, batched_inputs, backbone_feats, images_whwh, images, CbCr_vaild, filenames, clip_denoised=True, do_postprocess=True):
        ...
        if self.use_cbcr:
            img = self.get_batch_CbCr_sample(CbCr_vaild)
        else:
        ...
        for time, time_next in time_pairs:
            ...
            preds, outputs_class, outputs_coord = self.model_predictions(backbone_feats, images_whwh, img, time_cond,
                                                                         self_cond, filenames, clip_x_start=clip_denoised)
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        """
        images, images_whwh, CbCr_vaild, filenames = self.preprocess_image(batched_inputs)
        ...
        if not self.training:
            results = self.ddim_sample(batched_inputs, features, images_whwh, images, CbCr_vaild, filenames)
            return results
    ...
    def prepare_diffusion_repeat(self, gt_boxes, CbCr_vaild):
        """
    ...
    def prepare_diffusion_concat(self, gt_boxes, CbCr_vaild, use_cbcr=True):
        # """
        # :param gt_boxes: (cx, cy, w, h), normalized
        # :param num_proposals:
        # """
        ...
        if use_cbcr:
            x = self.get_CbCr_sample(CbCr_vaild)
            t = torch.ones((1,), device=self.device).long() * 999
            noise = torch.randn(self.num_proposals, 4, device=self.device)

            _x = torch.clamp(x, min=-1 * self.scale, max=self.scale)
            _x = ((_x / self.scale) + 1) / 2.
            diff_boxes = box_cxcywh_to_xyxy(_x)
            return diff_boxes, noise, x, t
        else:
            t = torch.randint(0, self.num_timesteps, (1,), device=self.device).long()
            noise = torch.randn(self.num_proposals, 4, device=self.device)
    ...
    def prepare_targets(self, targets, CbCr_vaild):
        new_targets = []
        diffused_boxes = []
        noises = []
        x_list = []
        ts = []
        for targets_per_image, CbCr_v in zip(targets, CbCr_vaild):
            target = {}
            ...
            d_boxes, d_noise, x, d_t = self.prepare_diffusion_concat(gt_boxes, CbCr_v, self.use_cbcr)
        ...
        return new_targets, torch.stack(diffused_boxes), torch.stack(noises), torch.stack(x_list), torch.stack(ts)
    ...
    def preprocess_image(self, batched_inputs):
        ...
        for bi in batched_inputs:
            ...
            CbCr_vaild.append(bi['vaild'])
            filenames.append(bi['file_name'])
            continue
        images_whwh = torch.stack(images_whwh)

        return images, images_whwh, CbCr_vaild, filenames
